            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
              ...
              <div className="overflow-hidden rounded-xl border border-slate-100 shadow-sm">
                <table className="w-full technical-table text-left border-collapse">
                  <thead>
                    <tr>
                      <th className="w-16 text-center py-2 text-[14px] bg-[#74045F] text-[#FFFFFF]">ลำดับที่</th>
                      <th className="text-center py-2 text-[14px] bg-[#74045F] text-[#FFFFFF]">รายการการตรวจสอบ</th>
                      <th className="w-1/3 text-center py-2 text-[14px] bg-[#74045F] text-[#FFFFFF]">ค่าผลลัพธ์ของอุปกรณ์</th>
                    </tr>
                  </thead>
                  <tbody>
                    {Object.entries(inspection.formData || {}).length > 0 ? (
                      Object.entries(inspection.formData || {}).map(([key, value]: [string, any], index) => (
                        <tr key={key} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}>
                          <td className="text-[12px] text-center text-[#000000] tracking-tighter py-2">
                            {String(index + 1).padStart(2, '0')}
                          </td>
                          <td className="text-[12px] text-center text-[#000000] py-2">
                            {getFieldLabel(key)}
                          </td>
                          <td className="text-[12px] text-center text-[#000000] bg-slate-50/50 py-2">
                      ...
                      <tr>
                        <td colSpan={3} className="py-20 text-center">
                          <div className="flex flex-col items-center gap-2 opacity-30">
                            <Info size={32} />
                            <p className="text-[12px] font-black uppercase tracking-widest text-slate-400">No field data records found for this audit.</p>
                          </div>
                        </td>
                      </tr>
                    )}
