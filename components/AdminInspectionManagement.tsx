...
export const AdminInspectionManagement: React.FC<AdminInspectionManagementProps> = ({
  ...
  const handleOpenBatchDeleteModal = (type: 'SELECTED' | 'ALL', currentFilteredItems: any[]) => {
    if (!isAdmin) {
      showToast('เฉพาะผู้ดูแลระบบ (Admin) เท่านั้นที่มีสิทธิ์ลบข้อมูล', 'error');
      return;
    }
    ...
    if (type === 'ALL' && currentFilteredItems.length === 0) {
  ...
  const handleConfirmBatchDelete = async () => {
    ...
    if (activeTab === 'REQUESTS') {
      let itemsToDelete = requests.filter(r => selectedIds.includes(r.id));
      if (batchDeleteType === 'ALL') {
        itemsToDelete = [...filteredRequests];
      }
      if (itemsToDelete.length === 0) {
        setIsBatchDeleteModalOpen(false);
        return;
      }
      const idsToRemove = new Set(itemsToDelete.map(r => r.id));
      ...
      try {
        for (const item of itemsToDelete) {
          await deleteDoc(doc(db, 'inspectionRequests', item.id));
        }
      } catch (err) {
        console.error("Batch delete inspectionRequests error:", err);
      }
      showToast(`ลบรายการคำร้องเรียบร้อยแล้วจำนวน ${itemsToDelete.length} รายการ`, 'success');
      setSelectedIds(prev => prev.filter(id => !idsToRemove.has(id)));
    } else if (activeTab === 'RESULTS') {
      let itemsToDelete = inspections.filter(i => selectedIds.includes(i.id));
      if (batchDeleteType === 'ALL') {
        itemsToDelete = [...filteredInspections];
      }
      if (itemsToDelete.length === 0) {
        setIsBatchDeleteModalOpen(false);
        return;
      }
      const idsToRemove = new Set(itemsToDelete.map(i => i.id));
      const updated = inspections.filter(i => !idsToRemove.has(i.id));
      setInspections(updated);
      safeSetLocalStorage('app_inspections', updated);
      ...
      try {
        ...
        console.error("Batch delete inspections error:", err);
      }
      showToast(`ลบรายการผลการตรวจสอบเรียบร้อยแล้วจำนวน ${itemsToDelete.length} รายการ`, 'success');
      setSelectedIds(prev => prev.filter(id => !idsToRemove.has(id)));
    }
